...
export default function ThemeSwitcher({ vertical = false }) {
    ...
    const themes = [
        ...
        { id: 'teal', color: '#028A88', label: 'Teal' },
    ];
                ...
                <>
                    ...
                    <div
                        className={`absolute z-50 bg-[#13161f] rounded-xl shadow-xl border border-white/10 p-2 animate-in fade-in slide-in-from-top-2 ${vertical
                            ? 'left-0 mt-2 w-48 flex flex-col gap-2 p-2'
                            : 'right-0 mt-2 w-60'
                            }`}
                    >
                        {!vertical && <div className="text-xs font-medium text-gray-400 px-2 py-1 mb-2">Select Theme</div>}
                        <div className={`${vertical ? 'flex flex-col gap-2' : 'grid grid-cols-5 gap-2 px-2 pb-2'}`}>
                            {themes.map((theme) => (
                                <button
                                    key={theme.id}
                                    onClick={() => {
                                ...
                                >
                                    <div
                                        className={`w-8 h-8 rounded-full border-2 transition-transform group-hover:scale-110 ${currentTheme === theme.id ? 'border-white ring-2 ring-white/20' : 'border-transparent'}`}
                                        style={{ backgroundColor: theme.color }}
                                    />
                                    {vertical && (
                                        <span className={`text-sm ${currentTheme === theme.id ? 'text-white font-medium' : 'text-gray-400 group-hover:text-white'}`}>
                                            {theme.label}
                                        </span>
                                    )}
                                </button>
